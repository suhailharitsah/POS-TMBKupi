...
<body class="min-h-screen flex items-center justify-center coffee-pattern p-4">
    ...
    <div class="w-full lg:w-1/2 p-6 sm:p-8 md:p-10 lg:p-12 xl:p-16 flex flex-col justify-center">
      ...
      <div class="text-center mb-6 md:mb-8">
        ...
        <h1
          ...
          TMBKupi</h1>
        <p class="text-gray-600 text-sm sm:text-base md:text-lg mt-2 font-medium">Bergabung Bersama Kami</p>
        <p class="text-gray-500 text-xs sm:text-sm mt-1">Buat akun baru untuk sistem kasir</p>
      </div>
      ...
      <form action="{{ route('register.post') }}" method="POST" class="space-y-4 md:space-y-5">
        ...
        <!-- Nama Lengkap -->
        <div>
          <label for="name" class="block text-sm md:text-base font-semibold text-gray-700 mb-2">Nama Lengkap</label>
          <div class="relative">
        ...
        <div>
          <label for="password_confirmation"
            class="block text-sm md:text-base font-semibold text-gray-700 mb-2">Konfirmasi Password</label>
          <div class="relative">
            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
              <svg class="h-5 w-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <input type="password" name="password_confirmation" id="password_confirmation" required
              class="input-focus w-full pl-10 pr-12 py-3 md:py-4 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-orange-400 focus:border-orange-400 text-sm md:text-base focus:outline-none"
              placeholder="••••••••" aria-describedby="togglePasswordConfDesc">

            <button type="button" id="togglePasswordConf"
              class="absolute inset-y-0 right-0 pr-3 flex items-center focus:outline-none hover:text-orange-500 transition-colors"
              aria-label="Tampilkan password konfirmasi" title="Tampilkan / sembunyikan password">
              <svg id="iconEyeConf" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-400"
                viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path d="M1 12s4-7 11-7 11 7 11 7-4 7-11 7S1 12 1 12z" />
                <circle cx="12" cy="12" r="3" />
              </svg>
              <svg id="iconEyeOffConf" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-400 hidden"
                viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path d="M17.94 17.94A10.94 10.94 0 0 1 12 19c-7 0-11-7-11-7a19.15 19.15 0 0 1 5.13-6.02" />
                <path d="M1 1l22 22" />
                <path d="M9.88 9.88A3 3 0 0 0 14.12 14.12" />
          ...
          <p id="togglePasswordConfDesc" class="sr-only">Tombol untuk menampilkan atau menyembunyikan password
            konfirmasi</p>
        </div>

        <!-- Register Button -->
        <button type="submit"
          class="btn-hover w-full bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white font-semibold rounded-xl py-3 md:py-4 transition duration-200 text-sm md:text-base shadow-lg mt-6">
          Daftar Sekarang
        </button>
      </form>
      ...
      <p class="text-center text-xs sm:text-sm md:text-base text-gray-600">
        Sudah punya akun?
        <a href="{{ route('login') }}"
          class="text-orange-500 hover:text-orange-600 font-semibold hover:underline transition-colors ml-1">Login
          sekarang</a>
      </p>
    ...
    <div class="hidden lg:flex lg:w-1/2 image-overlay relative">
      ...
      <div class="absolute inset-0 z-10 flex flex-col items-center justify-center p-12 text-white">
        <div class="text-center space-y-6 max-w-md">
          <h2 class="text-3xl xl:text-4xl font-bold drop-shadow-lg">Mulai Kelola Bisnis Anda</h2>
          <p class="text-base xl:text-lg drop-shadow-lg opacity-90">
            Bergabunglah dengan ratusan pemilik coffee shop yang sudah mempercayai sistem kami
          </p>
          ...
          <div class="space-y-4 mt-8">
            <div class="bg-white/20 backdrop-blur-sm rounded-xl p-4 text-left">
              <div class="flex items-center space-x-3">
                ...
                <div class="text-sm xl:text-base">Manajemen transaksi mudah</div>
              </div>
            </div>
            ...
            <div class="bg-white/20 backdrop-blur-sm rounded-xl p-4 text-left">
              <div class="flex items-center space-x-3">
                ...
                <div class="text-sm xl:text-base">Laporan penjualan real-time</div>
              </div>
            </div>
            ...
            <div class="bg-white/20 backdrop-blur-sm rounded-xl p-4 text-left">
              <div class="flex items-center space-x-3">
                ...
                <div class="text-sm xl:text-base">Kontrol inventory otomatis</div>
              </div>
            </div>
          </div>
        </div>
  ...
  <script>
    (function() {
      ...
      const toggleBtnConf = document.getElementById('togglePasswordConf')
      const iconEyeConf = document.getElementById('iconEyeConf')
      const iconEyeOffConf = document.getElementById('iconEyeOffConf')
      ...
      toggleBtnConf.addEventListener('click', function() {
        ...
        if (isPassword) {
          iconEyeConf.classList.add('hidden')
          iconEyeOffConf.classList.remove('hidden')
          toggleBtnConf.setAttribute('aria-label', 'Sembunyikan password konfirmasi')
          toggleBtnConf.title = 'Sembunyikan password'
        } else {
          iconEyeConf.classList.remove('hidden')
          iconEyeOffConf.classList.add('hidden')
          toggleBtnConf.setAttribute('aria-label', 'Tampilkan password konfirmasi')
          toggleBtnConf.title = 'Tampilkan password'
        }
      })

      toggleBtnConf.addEventListener('keydown', function(e) {
        if (e.key === ' ' || e.key === 'Enter') {
          e.preventDefault()
          toggleBtnConf.click()
        }
    ...
    // 🔹 SweetAlert2 Notifikasi Registrasi
    @if (session('success'))
      Swal.fire({
        icon: 'success',
        title: 'Berhasil!',
